...
import { getGenres, discoverTv, searchTv } from '../api'
...
export default function Series() {
  ...
  const genreList = genresFetch.data?.tv || []

  const queryTrim = query.trim()

  // Discover when filtering, /search/tv when a name is typed. Filtering uses a
  // text search can't also apply genre/language, so a typed query takes over.
  const buildRequest = (p) =>
    queryTrim
      ? searchTv(queryTrim, p)
      : discoverTv({
  ...
  return (
    <div className="series-page">
      ...
      {base.error && (
        <div className="error-box">Failed to load series: {base.error.message}</div>
      )}
      {!base.loading && !base.error && items.length === 0 && (
        <div className="search-empty">
          {queryTrim ? `No series found for "${queryTrim}".` : 'No series found.'}
        </div>
      )}
      ...
      {items.length > 0 && (
        <>
          <div className="series-grid">
            {items.map((s) => (
              <MovieCard key={s.id} item={{ ...s, media_type: 'tv' }} type="tv" />
            ))}
          </div>
          <div className="series-load">
            ...
            >
              {loadingMore ? 'Loading…' : hasMore ? 'Load More' : 'No more series'}
            </button>
          </div>
        </>
      )}
